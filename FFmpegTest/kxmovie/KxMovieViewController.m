...
//                          forState:UIControlStateNormal];
    ...
    UIImage *img_full = [UIImage imageNamed:@"fullscreen-button"];
    _fullscreenBtn = [[UIBarButtonItem alloc] initWithImage: img_full
                                                        style:UIBarButtonItemStylePlain
                                                        target:self
                                                        action:@selector(rotateDidTouch:)];
    UIImage *img_min = [UIImage imageNamed:@"minimize-button"];
    _minimizeBtn=   [[UIBarButtonItem alloc] initWithImage: img_min
                                                     style:UIBarButtonItemStylePlain
                                                    target:self
                                                    action:@selector(rotateDidTouch:)];
...
- (void) updateBottomBar
{
    ...
    UIBarButtonItem *rotateBtn    =self.fullScreenModeToggled ? _minimizeBtn : _fullscreenBtn;
    [_bottomBar setItems:@[_spaceItem, _rewindBtn, _fixedSpaceItem, playPauseBtn,
                           _fixedSpaceItem, _fforwardBtn,_spaceItem,rotateBtn,] animated:NO];
}
...
- (void) showHUD: (BOOL) show
{
    ...
    if (!_hiddenHUD) {
        [self performSelector:@selector(showHUD:) withObject:nil afterDelay:10.0f];
    }
